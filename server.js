...
const BLACKCAT_API_URL = String(process.env.BLACKCAT_API_URL || 'https://api.blackcatoficial.com/api').replace(/\/+$/, '');
...
app.post('/api/pix', limitPixRequests, async (req, res) => {
  ...
  try {
    ...
    if (!apiKey) {
      console.error(`[${requestId}] API Key da BlackCat ausente.`);
    ...
    const gatewayResponse = await axios.post(`${BLACKCAT_API_URL}/sales/create-sale`, payload, {
      headers: {
        'X-API-Key': apiKey,
    ...
    const paymentData = transaction && transaction.paymentData;
    const pixCode = paymentData && (paymentData.copyPaste || paymentData.qrCode);

    if (!pixCode || typeof pixCode !== 'string') {
      console.error(`[${requestId}] Resposta sem paymentData.copyPaste ou paymentData.qrCode.`, {
    ...
    return res.json({
      success: true,
      transactionId: String(transaction.transactionId || externalRef),
      pixCode,
    ...
    console.error(`[${requestId}] Erro ao gerar PIX na BlackCat (${gatewayStatus || 'sem status'}): ${gatewayMessage}`);
